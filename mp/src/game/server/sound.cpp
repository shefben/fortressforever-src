...
void SENTENCEG_PlaySentenceIndex( edict_t *entity, int iSentenceIndex, float volume, soundlevel_t soundlevel, int flags, int pitch )
{
	if ( iSentenceIndex >= 0 )
	{
		...
		CBasePlayer* pPlayer = ToBasePlayer(GetContainingEntity( entity ), soundlevel );
		if (pPlayer)
		{
			CSingleUserRecipientFilter filter(pPlayer);
			// Using CHAN_VOICE, the vox was getting cut-off by pain/death sounds
			CBaseEntity::EmitSentenceByIndex( filter, ENTINDEX(entity), CHAN_STREAM, iSentenceIndex, volume, soundlevel, flags, pitch );
		}
#endif
	}
}
