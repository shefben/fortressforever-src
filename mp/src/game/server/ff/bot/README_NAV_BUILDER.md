...
| Slot | Page 1 — objectives & build | Page 2 — tactical | Page 3 — occasional & links | Page 4 — doors, ladders & erase |
|:---:|---|---|---|---|
| 1 | `sniper` | `detpackseal` | `resupply` | `ladder` |
| 2 | `sentry` | `pipetrap` | `nospawn` | `grenades` |
| 3 | `dispenser` | `aim` | `escape` | `doorteam` |
| 4 | `detpack` | `defend` | `linkfrom` | `dooroneway` |
| 5 | `spawn` corner | `danger` | `linkto` | `erase` |
| 6 | `door` | `jump` | | |
| 7 | `flag` | `waterexit` | | |
| 8 | `cap` | `lift` | | |
...
### Page 4: the things that were detected but invisible

Ladders, doors and grenade crates were all being detected and tagged already,
and all three were invisible and unauthorable: no marker type, and in two cases
no entry in `ff_nav_visualize` either. That made "the ladder in 2fort has no nav
points on it" indistinguishable from three different problems — the ladder was
never generated, it was generated but connects to nothing, or it was tagged
perfectly and simply never drawn. Those want completely different fixes.

| Marker | Sets | Use it when |
|---|---|---|
| `ladder` | `FF_NAV_NEAR_LADDER` + `FF_NAV2_LADDER` | nav generation produced no ladder here. Stock Source builds **no ladders at all** outside Left 4 Dead, so a mesh generated by a build without FF's ladder pass has none anywhere |
| `grenades` | `FF_NAV_HAS_GRENADES` + `FF_NAV2_GRENADE_RESUPPLY` | there is a grenade crate here the map's Lua doesn't declare |
| `doorteam` | `FF_NAV_DOORWAY` + `FF_NAV2_DOOR_<team>` | a door that only opens for one team |
| `dooroneway` | ...plus `FF_NAV2_DOOR_ONEWAY` | a respawn gate: that team leaves through it, nobody comes in |

The two door types matter more than they sound. `FF_NAV_DOORWAY` on its own
means "an openable brush overlaps this area", which is enough to stop the path
cost treating a *shut* door as a wall — the bot walks up to it and opens it. A
team door cannot be opened by walking up to it, so keeping the route alive sends
the bot to stand in front of it indefinitely, and that looks for all the world
like a pathing bug rather than a permissions one. `CFFBotPathCost` refuses a
door whose team bits don't include yours outright.

Most of this is auto-detected. `CFFBotAutoTagger::TagTeamDoors` reads the shape
of the situation rather than the entity: a door whose areas straddle *exactly
one* team's spawn room and the world outside it is that team's respawn gate,
whatever the script calls it. FF drives door permissions from Lua and the brush
is a plain `func_door`, so there is nothing on the entity to read; the targetname
is used only as a fallback, because it is a convention rather than data.

### `erase` — deleting what the generator produced

The other four pages all *add* something. This one takes it away, and it exists
because taking something away turned out to be the harder problem.

Nothing the tagger, the auto-tagger or the analyzer produces is stored anywhere.
All of it is recomputed from live entities and mesh geometry at every map load.
So "delete this auto-generated sniper spot" cannot be done by clearing a bit —
the bit is back a moment later, and back again on every subsequent load. What
*can* persist is the decision, which is what this marker is: a record that an
author looked at this spot and said no.

It is applied dead last, after every derivation pass, for the same reason.
Applied any earlier and the pass that produced the tag simply produces it again
within the same map load — which is indistinguishable from the delete having
silently failed.

An erase clears both attribute words completely, with one exception:
`FF_NAV_BLOCKED` is not a tag anybody authored. It is set and cleared at runtime
by `CNavArea::UpdateBlocked` from door state, and clearing it would tell the
pathfinder a shut door is open until the next time the door moves.

Delete the erase marker to bring the derived tags back.
...
`KP_DEL` or `ff_nav_delete [radius]` removes **the single nearest** nav point
within `ff_nav_builder_delete_radius` (default 192u).

Nearest rather than everything-in-range, because deletion is usually a
correction: you placed one marker slightly wrong and want *that one* gone. In a
doorway with a door marker, a defend marker and two spawn corners inside 96
units of each other, "delete everything near me" takes four markers when you
meant one, and three of them were fine. `ff_nav_delete_all [radius]` is the old
behaviour, for clearing an area out wholesale.

**Distance is measured from the floor below you, not from the camera.** Same
trace placement uses, and for the same reason: authoring is almost always done
from noclip or from the spectator camera, where your origin is somewhere in the
air above the thing you are looking at. Measuring from the camera means the
marker you are floating directly over is several hundred units away and no
sensible radius reaches it. Delete where you would place — anything else makes
the two commands disagree about what "here" means.

### Deleting something you didn't place

If there is no hand-placed marker in range, `ff_nav_delete` **erases the derived
tags on the nav area you are over** instead, by dropping an `erase` marker there.

That is the only thing that can work. Everything the tagger, the auto-tagger and
the analyzer produce is recomputed from entities and geometry at every map load,
so clearing a bit lasts until the next load and no longer. See the `erase`
section above for what it clears and what it deliberately doesn't.

It tells you what it removed:

```
[ff_nav_delete] erased area 812 (was attr1=0x08040000 attr2=0x00060000).
Delete the erase marker to bring them back.
```

Set `ff_nav_delete_erases_derived 0` if you only ever want to remove your own
markers.
...
| `ff_nav_builder_delete_radius` | `192` | How close a marker must be for `ff_nav_delete`, measured from the floor below you |
| `ff_nav_delete_erases_derived` | `1` | What `ff_nav_delete` does with no marker in range. `1` = erase the derived tags on the area under you and keep them erased; `0` = do nothing |
