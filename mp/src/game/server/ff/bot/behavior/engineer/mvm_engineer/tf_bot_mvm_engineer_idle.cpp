...
#include "bot/map_entities/ff_bot_hint_engineer_nest.h"
...
#include "bot/behavior/engineer/mvm_engineer/ff_bot_mvm_engineer_build_teleporter.h"
...
ConVar ff_bot_engineer_mvm_sentry_hint_bomb_forward_range( "ff_bot_engineer_mvm_sentry_hint_bomb_forward_range", "0", FCVAR_CHEAT );
ConVar ff_bot_engineer_mvm_sentry_hint_bomb_backward_range( "ff_bot_engineer_mvm_sentry_hint_bomb_backward_range", "3000", FCVAR_CHEAT );
ConVar ff_bot_engineer_mvm_hint_min_distance_from_bomb( "ff_bot_engineer_mvm_hint_min_distance_from_bomb", "1300", FCVAR_CHEAT );
...
bool GetBombInfo( BombInfo_t* pBombInfo = NULL )
{
	...
	float flMaxBattlefront = battlefront + ff_bot_engineer_mvm_sentry_hint_bomb_backward_range.GetFloat();
	float flMinBattlefront = battlefront - ff_bot_engineer_mvm_sentry_hint_bomb_forward_range.GetFloat();
...
bool CFFBotMvMEngineerHintFinder::FindHint( bool bShouldCheckForBlockingObjects, bool bAllowOutOfRangeNest, CHandle< CFFBotHintEngineerNest >* pFoundNest /*= NULL*/ )
{
	...
	for( int i=0; i<activeEngineerNest.Count(); ++i )
	{
		...
		if ( hintDistanceToTarget > bombInfo.m_flMinBattleFront && hintDistanceToTarget < bombInfo.m_flMaxBattleFront )
		{
			...
			{
				if ( VectorLength( bombInfo.m_vPosition - vNestPosition ) < ff_bot_engineer_mvm_hint_min_distance_from_bomb.GetFloat() )
				{
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( ff_bot_mvm_show_engineer_hint_region, "Show the nav areas MvM engineer bots will consider when selecting sentry and teleporter hints", FCVAR_CHEAT )
{
	...
	if ( result.DidHit() )
	{
		...
		if ( area )
		{
			...
			float maxBattlefront = battlefront + ff_bot_engineer_mvm_sentry_hint_bomb_backward_range.GetFloat();
			float minBattlefront = battlefront - ff_bot_engineer_mvm_sentry_hint_bomb_forward_range.GetFloat();
			...
			for( int i=0; i<battlefrontAreaVector.Count(); ++i )
			{
				...
				for ( int j=0; j<hintAreaVector.Count(); ++j )
				{
					if ( fillArea == hintAreaVector[j] )
					{
						...
						{
							bool bTooCloseToBomb = VectorLength( result.endpos - pHint->GetAbsOrigin() ) < ff_bot_engineer_mvm_hint_min_distance_from_bomb.GetFloat();
			...
			NDebugOverlay::Sphere( result.endpos, ff_bot_engineer_mvm_hint_min_distance_from_bomb.GetFloat(), 255, 255, 0, false, flDrawTime );
		}
	}
}
