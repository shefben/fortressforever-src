...
ConVar ff_bot_engineer_mvm_building_health_multiplier( "ff_bot_engineer_building_health_multiplier", "2", FCVAR_CHEAT );
...
ActionResult< CFFBot >	CFFBotMvMEngineerBuildTeleportExit::Update( CFFBot *me, float interval )
{
	...
	else if ( m_delayBuildTime.IsElapsed() )
	{
		...
		if ( myTeleporter )
		{
			...
			int iHealth = myTeleporter->GetMaxHealthForCurrentLevel() * ff_bot_engineer_mvm_building_health_multiplier.GetFloat();
