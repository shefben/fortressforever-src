...
#include "bot/behavior/engineer/ff_bot_engineer_build_sentrygun.h"
#include "bot/behavior/engineer/ff_bot_engineer_build_dispenser.h"
...
ConVar ff_bot_engineer_retaliate_range( "ff_bot_engineer_retaliate_range", "750", FCVAR_CHEAT, "If attacker who destroyed sentry is closer than this, attack. Otherwise, retreat" );
ConVar ff_bot_engineer_exit_near_sentry_range( "ff_bot_engineer_exit_near_sentry_range", "2500", FCVAR_CHEAT, "Maximum travel distance between a bot's Sentry gun and its Teleporter Exit" );
ConVar ff_bot_engineer_max_sentry_travel_distance_to_point( "ff_bot_engineer_max_sentry_travel_distance_to_point", "2500", FCVAR_CHEAT, "Maximum travel distance between a bot's Sentry gun and the currently contested point" );
...
*/
	...
	else if ( m_teleportExitRetryTimer.IsElapsed() && myTeleportEntrance && !isUnderAttack )
	{
		...
		if ( m_sentryBuildHint != NULL )
		{
			...
			{
				mySentry->UpdateLastKnownArea();
				CBaseEntity *closeHint = SelectClosestEntityByTravelDistance( me, hintVector, mySentry->GetLastKnownArea(), ff_bot_engineer_exit_near_sentry_range.GetFloat() );
