...
enum FFNavAttributeType2
{
	...
	// Traversal furniture.
	//
	// FF_NAV_NEAR_LADDER and FF_NAV_DOORWAY already existed, but both are
	// invisible: neither has a marker type, so neither can be authored where
	// the auto-detection misses one, and neither appeared in ff_nav_visualize,
	// so "2fort's ladder has no nav points on it" was indistinguishable from
	// "the ladder was never tagged". These are the authorable, visible
	// versions, plus the two distinctions the old single DOORWAY bit could not
	// express.
	//-------------------------------------------------------------------------

	// This area is a ladder endpoint. Set alongside FF_NAV_NEAR_LADDER by the
	// auto-tagger and placeable by hand for the ladders nav_generate misses —
	// stock Source builds no ladders at all outside Left 4 Dead, so a mesh
	// generated by a build without FF's ladder pass has none.
	FF_NAV2_LADDER						= 0x00800000,

	// A door that only opens for one team, and only from one side: a respawn
	// room gate. You can leave through it; you cannot come back in, and the
	// enemy cannot come in at all.
	//
	// This is not the same claim as FF_NAV_DOORWAY, which only says "a
	// brush entity that opens overlaps this area". Pathing through an enemy
	// spawn gate is not expensive, it is impossible, and costing it as merely
	// expensive is how a bot ends up standing in a doorway that will never
	// open for it.
	FF_NAV2_DOOR_ONEWAY					= 0x01000000,

	// Per-team door ownership. Set together with either DOOR_ONEWAY (a spawn
	// gate) or on its own (a team-restricted door in the middle of a map).
	FF_NAV2_DOOR_BLUE					= 0x02000000,
	FF_NAV2_DOOR_RED					= 0x04000000,
	FF_NAV2_DOOR_YELLOW					= 0x08000000,
	FF_NAV2_DOOR_GREEN					= 0x10000000,
	FF_NAV2_DOOR_TEAM_ANY				= 0x1E000000,

	// Grenade resupply. FF_NAV_HAS_GRENADES has existed since the first
	// tagger and nothing has ever read it: the ammo behaviour looked for
	// FF_NAV_HAS_AMMO only, so a bot out of grenades would walk past the
	// grenade crate it was standing next to.
	FF_NAV2_GRENADE_RESUPPLY			= 0x20000000,

	// An author has explicitly wiped this area. Set by the 'erase' marker,
	// applied after every derivation pass, and honoured by all of them.
	//
	// It exists because everything else in both attribute words is re-derived
	// from entities and geometry at every map load. Without a record that
	// says "not here", deleting an auto-generated tag is not something that
	// can persist — it comes back on the next map load, every time.
	FF_NAV2_ERASED						= 0x40000000,
};
...
public:
	...
	// Convenience: per-team spawn-room / flag / cap / defend / door bit lookup.
	...
	static int		DoorAttributeForTeam( int team );

	// True if this area's door, if it has one, will open for the given team.
	// An area with no door bits is not a door and is therefore passable — the
	// question only means anything where a door was detected.
	bool			IsDoorPassableByTeam( int team ) const
	{
		if ( ( m_attributeFlags2 & FF_NAV2_DOOR_TEAM_ANY ) == 0 )
			return true;
		return ( m_attributeFlags2 & DoorAttributeForTeam( team ) ) != 0;
	}
