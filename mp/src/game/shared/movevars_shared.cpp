...
#else
ConVar	sv_maxspeed		("sv_maxspeed", "600", FCVAR_NOTIFY | FCVAR_REPLICATED);	// |-- Mirv: [600 is] a more sane max speed
#endif // CSTRIKE_DLL
...
ConVar	sv_wateraccelerate(  "sv_wateraccelerate", "10", FCVAR_NOTIFY | FCVAR_REPLICATED );     
ConVar	sv_waterfriction(  "sv_waterfriction", "1", FCVAR_NOTIFY | FCVAR_REPLICATED );      
ConVar	sv_footsteps	( "sv_footsteps", "1", FCVAR_NOTIFY | FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif, "Play footstep sound for players" );
ConVar	sv_rollspeed	( "sv_rollspeed", "200", FCVAR_NOTIFY | FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif);
ConVar	sv_rollangle	( "sv_rollangle", "0", FCVAR_NOTIFY | FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif, "Max view roll angle");
#endif // CSTRIKE_DLL

#if defined( DOD_DLL ) || defined( CSTRIKE_DLL ) || defined( HL1MP_DLL ) || defined( HL2MP_DLL ) || defined( TF_DLL )
...
#else
ConVar	sv_bounce		( "sv_bounce","0", FCVAR_NOTIFY | FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif, "Bounce multiplier for when physically simulated objects collide with other objects." );
ConVar	sv_maxvelocity	( "sv_maxvelocity","3500", FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif, "Maximum speed any ballistically moving object is allowed to attain per axis." );
ConVar	sv_stepsize		( "sv_stepsize","18", FCVAR_NOTIFY | FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif);
ConVar	sv_backspeed	( "sv_backspeed", "0.6", FCVAR_ARCHIVE | FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif, "How much to slow down backwards motion" );
ConVar  sv_waterdist	( "sv_waterdist","12", FCVAR_REPLICATED #ifdef FF_DLL | FCVAR_DEVELOPMENTONLY #endif, "Vertical view fixup when eyes are near water plane." );
#endif // CSTRIKE_DLL
